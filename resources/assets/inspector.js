    ...
    function renderAppProfile(app) {
      ...
      const iconHtml = app.icon ? `<img src="/${esc(app.icon)}" class="h-16 w-16 rounded-2xl object-cover" alt="">` : `<div class="grid h-16 w-16 place-items-center rounded-2xl border border-violet-300/20 bg-violet-400/10 text-violet-200">${icon('package', 'h-8 w-8')}</div>`;
      ...
      $('themesContent').innerHTML = themes.length ? `
        <table class="w-full min-w-[980px] text-left text-sm">
          ...
          <tbody class="divide-y divide-white/10">${themes.map((theme, index) => `<tr onclick="selectTheme(${index})" class="cursor-pointer transition hover:bg-white/[.05] ${index === state.selectedTheme ? 'bg-violet-500/16' : ''}">
            <td class="px-4 py-4"><div class="flex items-center gap-4"><div class="grid h-16 w-24 shrink-0 place-items-center overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-violet-500/25 via-sky-500/10 to-emerald-500/15 text-xs font-bold text-slate-300">${theme.preview ? `<img src="/${esc(theme.preview)}" class="h-full w-full object-cover" alt="">` : esc(theme.title || theme.name)}</div><div class="min-w-0"><div class="flex items-center gap-2"><span class="truncate font-bold text-white">${esc(theme.title || theme.name)}</span>${theme.active ? '<span class="rounded-lg bg-emerald-400/12 px-2 py-1 text-xs font-bold text-emerald-300">Active</span>' : ''}</div><div class="mt-1 line-clamp-2 text-xs text-slate-400">${esc(theme.description || '')}</div></div></div></td>
    ...
    function renderThemeDetails(theme) {
      ...
      $('themeDetails').innerHTML = `
        <div class="rounded-3xl border border-white/10 bg-[#091320]/90 p-4 shadow-[0_18px_70px_rgba(0,0,0,.22)]">
          ...
          <div class="mb-4 h-36 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-violet-500/20 via-sky-500/10 to-emerald-500/10">${theme.preview ? `<img src="/${esc(theme.preview)}" class="h-full w-full object-cover" alt="">` : ''}</div>
    ...
    function renderBuildSigning(sign) {
      ...
      return `
        ...
        <p class="mt-4 rounded-2xl border border-white/10 bg-white/[.03] p-3 text-sm leading-6 text-slate-500">The generated key is stored under <code>storage/pinx/signing</code> and <code>app.php</code> is updated in <code>pinx.sign</code>. Keep production signing keys outside public source control.</p>
      `;
    }
